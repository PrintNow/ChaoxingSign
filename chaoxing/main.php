...
foreach ($taskID as $k => $v) {
    ...
    $signRes = trim(curl_get(sprintf(SIGN_API, $v[2],$enc), $jar_path));//签到结果
...
if (!isset($login_data['status'])) {
    die("登陆失败，原因：API 错误，请再次尝试。多次出现此问题请前往 https://github.com/ 提交 Issues");
}
